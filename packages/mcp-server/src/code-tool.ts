...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ...
    BEAGLE_BASE_URL:
      readEnv('BEAGLE_BASE_URL') ?? readEnv('BEAGLE_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
  };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      ...
      code,
      intent,
      client_opts: { environment: (readEnv('BEAGLE_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@corgi-tech/beagle';
