...
export { ClientOptions } from '@corgi-tech/beagle';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/beagle',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the beagle MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new Beagle({
    ...{ environment: (readEnv('BEAGLE_ENVIRONMENT') || undefined) as any },
    logger,
